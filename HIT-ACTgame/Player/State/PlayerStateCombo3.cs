...
public class PlayerStateCombo3 : PlayerStateBase
{
    ...
    public override void OnInit()
    {
        ...
        playerState = PlayerState.Combo3; //记录状态
        aniName = "Combo3"; //记录动画名
    }
    ...
    public override void OnControl()
    {
        ...
        if (animator.GetCurrentAnimatorStateInfo(0).normalizedTime > 0.4f && nextCombo)
        {
            if (player.StateActionCheck(PlayerState.Combo4))
            {
                //切换到连击4状态
                manager.ChangeState<PlayerStateCombo4>();
                return;
            }
        }
    ...
    public override void OnExcute()
    {
        ...
        //相机震动
        if (animator.GetCurrentAnimatorStateInfo(0).normalizedTime > 0.25f &&
            animator.GetCurrentAnimatorStateInfo(0).normalizedTime < 0.35f)
            camera.Shake(0.02f, 6.0f, 0.002f, 0.3f);
    }
